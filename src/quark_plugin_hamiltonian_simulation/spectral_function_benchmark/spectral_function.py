...
from .spectral_function_score import extract_simulation_results,exact_values,compute_score
...
class SpectralFunction(Core):
    ...
    def preprocess(self, data: InterfaceType) -> Result:
        ...
        if(not(self.mode == 'log2N' or self.mode=='CZ_pyramid' or self.mode=='CX_ladder')):
            raise ValueError('mode must be either log2N (default), CZ_pyramid or CX_ladder')

        U_list=create_sequence_circuit(self.epsilon,6/(self.l-1),self.l,self.dt,self.n_trot,self.mode,self.l)

        return Data(Other(AbstractCircuits([{'circuit':U_list[n], 'number_of_qubits':self.l*2, 'shot_proportions':1, 'n_shots':self.n_shots} for n in range(len(U_list))],self.benchmark_tag())))
    ...
    def postprocess(self, input_data: Other[BackendResult]) -> Result:
        ...
        exact=exact_values(self.l,self.dt,self.n_trot,self.epsilon,6/(self.l-1),self.l)

        fidelity,dispersion,dispersion_fidelity,dispersion_fidelity_exact=compute_score(results[:,:,0],exact,6/(self.l-1),self.l,self.l)

        logger.info(f"Benchmark score (global fidelity): {fidelity}")
        logger.info(f"Benchmark score (noiseless dispersion fidelity): {dispersion_fidelity}")
        logger.info(f"Benchmark score (exact dispersion fidelity): {dispersion_fidelity_exact}")
        self.metrics.update({
        ...
        if(self.create_plot):
            ...
            axarr[0].imshow(results[:,:,0],aspect=self.l/6,extent=[0,self.l,-3,3],origin='lower',vmin=0,vmax=1)
            axarr[0].plot(dispersion[:,0],dispersion[:,1],color='red')
            axarr[0].set_title("results")
            axarr[0].set_xlabel("k")
            axarr[0].set_ylabel("$\omega$")
            
            im1=axarr[1].imshow(exact,aspect=self.l/6,extent=[0,self.l,-3,3],origin='lower',vmin=0,vmax=1)
        ...
        return Data(Other({"application_score_fidelity": fidelity,
            "application_score_noiseless_dispersion": dispersion_fidelity,
            "application_score_exact_dispersion": dispersion_fidelity_exact,
                           }))
