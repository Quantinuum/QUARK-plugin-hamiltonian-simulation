...
class AerSimulator(Core):
    ...
    def preprocess(self, input_data: Other[BackendInput]) -> Result:
        ...
        shot_proportions = backend_input.shot_proportions
    ...
    pauli_error, depolarizing_error, thermal_relaxation_error)
            ...
            counts=backend.run(circuit,noise_model=noise_model, shots=max(round(self.n_shots*shot_proportions[n]),1)).result().get_counts(circuit)
