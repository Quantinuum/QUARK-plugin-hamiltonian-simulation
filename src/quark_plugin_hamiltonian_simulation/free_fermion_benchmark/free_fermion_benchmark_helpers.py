...
def toric_code_preparation(u:list, lx: int, ly: int, periodic: bool, shift: int) -> None:
    ...
    for k in range(lyp//2-1): #sequence of CXs
        for shifting in [[0,2],[-1,1],[0,1]]:
            for j in range(lxp):
                f1=coordinates_ancilla(j,lyp-2*k-3,lxp,lyp,periodic)
                f2=coordinates_ancilla(j+shifting[0],lyp-2*k-3+shifting[1],lxp,lyp,periodic)
                if(f1>=0 and f2>=0):
    ...
    for k in range(lyp): #change of basis to make all the stabilizers into identical XXYY strings
        for j in range(lxp):
...
def inverse_toric_code_preparation(u:list, lx: int, ly: int, periodic: bool, shift: int) -> None:
...
def plaquette_trotter_step(u:list, theta: float, x0:int, x1:int, x2:int, x3:int, a:int, shift:int, shift_ancilla:int) -> None:
...
def trotter_step(u:list,dt:float,lx:int,ly:int,periodic:bool,two_spin_species:bool) -> None:
    ...
    for stage_species in range(spin_species):#on which spin species we act
        for stage in [0,1]:#whether 'P' or 'Q' plaquettes
            for k in range(-stage,ly//2):#loops over plaquettes. Negative values are for the non-periodic case where edge sites don't form a complete face
                for j in range(-1+stage,lx//2):
                    if(j>=0 or (not periodic)):
                        if(k>=0 or (not periodic)):
                            a=coordinates_ancilla(j,2*k+stage,lx//2,ly,periodic)
                            x0=coordinates(2*j+1-stage,2*k+stage,lx,ly,periodic)
                            x1=coordinates(2*j+2-stage,2*k+stage,lx,ly,periodic)
                            x2=coordinates(2*j+1-stage,2*k+1+stage,lx,ly,periodic)
                            x3=coordinates(2*j+2-stage,2*k+1+stage,lx,ly,periodic)

                            plaquette_trotter_step(u,dt,x0,x1,x2,x3,a,
                                                ((stage_species+stage)%2)*(spin_species-1)*lx*ly,shift_ancilla)
        if(spin_species==2):
...
def adjacent_ancilla(j0:int,j1:int,lx:int,ly:int,periodic:bool) -> int:
    ...
    if(x1==x0+1):
        a=coordinates_ancilla(x0//2,y0-((x0+y0+1)%2),lx//2,ly,periodic)
    elif(y1==y0+1):
...
def initialize_product_state(u:list,initial:list, lx:int, ly:int, periodic:bool, two_spin_species:bool) -> None:
    ...
    if(two_spin_species): #if there are two spin species, the initialization is trivial and does not require acting on the ancillas as it commutes with the stabilizers
        ...
        while(len(initial_editable)>0):
            x0=initial_editable.pop() #take one vertex on which we must put a fermion
...
def create_circuit(lx: int, ly: int, dt: float,periodic:bool,two_spin_species:bool, n_trot: int) -> list:
