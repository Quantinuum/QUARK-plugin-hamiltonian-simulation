...
from .free_fermion_benchmark_helpers import adjacent_ancilla,toric_code_preparation,change
    ...
    for y in range(ly):#loops over all odd faces of the square lattice
        for x in range(lx//2):
            if(periodic or (y<ly-1 and (x+y%2)<lx//2)):
                ...
                s=change(s,x2,'Z')
                s=change(s,x3,'Z')
                if(two_spin_species): #if two species are present, couples to other other species as well
                    s=change(s,x0+lx*ly,'Z')
                    s=change(s,x1+lx*ly,'Z')
                    s=change(s,x2+lx*ly,'Z')
                    s=change(s,x3+lx*ly,'Z')
                a=adjacent_ancilla(x0,x1,lx,ly,periodic) #adds ancilla adjacent to x0-x1, etc
                if(a>=0):
...
def evolve(string:str,operation:list) -> tuple[str,float]:
    ...
    if(name=='x'):
        ...
        result=str(string)
        coeff=1
        if(not((string[j]=='I' or string[j]=='Z')and(string[k]=='I' or string[k]=='X'))):
            if(string[j]=='I'):
                if(string[k]=='Y'):
                    result=change(string,j,'Z')
                    coeff=1
...
def stabilizers_after_toric_code(lx:int,ly:int,periodic:bool,two_spin_species:bool) -> list:
    '''computes the list of stabilizers after applying the toric code state preparation'''
    S=stabilizers(lx,ly,periodic,two_spin_species) #list of stabilizers before toric code state preparation
    U=[]
    if(two_spin_species):
        shift=2*lx*ly
    else:
    ...
    toric_code_preparation(U,lx,ly,periodic,shift) #U is the circuit that prepares the toric code
    ...
    for s in S:
        ...
        coeff=1
        for g in U[::-1]:
            safter,multiplier=evolve(safter,g) #computes the Pauli string obtained after toric code preparation for every stabilizer
            coeff*=multiplier
