...
def createScheduling(partition_start:list,partition_end:list,n_trot:int,dt:float) -> list:
    '''returns the list of times used for the adiabatic evolution'''
    varphi=lambda x:(1+np.tanh(np.tan((x-0.5)*np.pi)))/2
    T=[]
    ...
    for t in range(n_trot):
        a=[0]*n
        for j in range(n):
            a[j]=(partition_start[j]+varphi((t+0.5)/n_trot)*(partition_end[j]-partition_start[j]))*dt
            if(decrease_dt):
...
def createCircuitSquareIsing(lx:int, ly:int, h:float, periodic:bool,  n_trot:int,dt:float)-> tuple[list,list]:
    ...
    shot_proportions=[abs(h)/(1+abs(h)),1/(1+abs(h))] #proportion of number of shots for every circuit

    for j in V:#appends a X on site j at the beginning of every circuit
        resU.append([['x',j]]+Uzz)
        resU.append([['x',j]]+Ux) 
        shot_proportions=shot_proportions+[abs(h)/(1+abs(h))/len(V),1/(1+abs(h))/len(V)]
