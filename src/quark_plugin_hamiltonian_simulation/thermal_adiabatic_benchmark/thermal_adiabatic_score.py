...
def extract_simulation_results_square_ising(lx: int, ly: int,periodic:bool, h:float,counts_per_circuit: list[dict[str, int]]) -> list:
    ...
    return [energy/len(V),var_energy/len(V),energy_excited/len(V),var_energy_excited/len(V),mirror/len(V),var_mirror/len(V),mirror_excited/len(V),var_mirror_excited/len(V)]
...
def thermodynamics(results:list) -> tuple:
    ...
    f=lambda e,ep,m,mp:2*(e-ep)/(m-mp)/np.log((1-m)/(1+m)) #function computing the temperature
    ...
    for n in range(10000):
        ...
        value=f(results[0]+results[1]*xi1,results[2]+results[3]*xi2,min(max(results[4]+results[5]*xi3,-0.99999999),0.99999999),min(max(results[6]+results[7]*xi4,-0.99999999),0.99999999))
    ...
    var_free_energy=np.sqrt(var_energy**2+temperature**2*var_entropy**2+var_temperature**2*entropy**2)

    return energy,var_energy,entropy,var_entropy,temperature,var_temperature,free_energy,var_free_energy
